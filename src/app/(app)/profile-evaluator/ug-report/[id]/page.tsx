...
import {
  ...
  Info, Star, Users, BarChart3, BookOpen
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { Student12thProfileReport } from "@/lib/ug-profile-engine";
...
export default function UGReportViewPage() {
  ...
  const [report, setReport] = useState<Student12thProfileReport | null>(null);

  useEffect(() => {
    setMounted(true);
    const saved = localStorage.getItem(`ug_profile_report_${reportId}`);
    if (saved) {
  ...
  const handleDownload = async () => {
    ...
    html2pdf()
      .set({
        margin: 0.5,
        filename: `UG_Profile_Report_${reportId}.pdf`,
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-12">
            ...
            <div className="relative w-full rounded-[32px] overflow-hidden bg-gradient-to-br from-[#2563EB] via-[#4F46E5] to-[#7C3AED] p-10 md:p-14 text-white shadow-2xl">
              ...
              <div className="relative z-10">
                <div className="flex flex-col md:flex-row gap-8 justify-between">
                  <div>
                    <p className="text-white/60 text-sm font-semibold uppercase tracking-widest mb-2">
                      UG Profile Intelligence
                    </p>
                    <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-3">
                      {report.academicCategory}
                    </h1>
                    <p className="text-white/75 text-base max-w-lg">Bachelor Admission Readiness & Pathway Assessment</p>
                  </div>
                  <div className="shrink-0 flex flex-col items-end justify-center gap-1">
                    <p className="text-white/60 text-xs uppercase tracking-widest">Readiness Score</p>
                    <p className="text-7xl font-black leading-none">{report.readinessScore}</p>
                ...
                <div className="mt-10 pt-8 border-t border-white/15 grid grid-cols-2 sm:grid-cols-4 gap-6">
                  <div>
                    <p className="text-white/50 text-xs uppercase tracking-widest mb-1">Target Degree</p>
                    <p className="font-semibold">{report.finalAnswers.degree.replace("Pursue ", "").replace(" based on your interest.", "")}</p>
                  </div>
                  <div>
                    <p className="text-white/50 text-xs uppercase tracking-widest mb-1">Academics</p>
                    <p className="font-semibold">{report.academicScore} / 100</p>
                  </div>
                  <div>
                    <p className="text-white/50 text-xs uppercase tracking-widest mb-1">Target Exams</p>
                    <p className="font-semibold truncate" title={report.finalAnswers.exams.replace("Focus on: ", "")}>
                      {report.finalAnswers.exams.replace("Focus on: ", "").replace(".", "")}
                    </p>
                  </div>
                  <div>
          ...
          <motion.section variants={itemVariants}>
            <div className={sectionHeader}>
              ...
              <h2 className="text-xl font-bold text-[#111827] dark:text-white">Score Breakdown</h2>
            </div>
            <div className={`${card} p-8 space-y-6`}>
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xs text-[#6B7280]">10th (40%) + 12th (60%) weighted</span>
                </div>
                <ScorePill score={report.academicScore} />
              </div>

              <div className="pt-4 border-t border-black/5 dark:border-white/5">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-bold text-[#111827] dark:text-white">Bachelor Admission Readiness</span>
                  <span className="text-2xl font-black text-[#4F46E5]">{report.readinessScore}</span>
                </div>
          ...
          {/* ══ SECTION 3: ENTRANCE EXAM ANALYSIS ══ */}
          <motion.section variants={itemVariants}>
            <div className={sectionHeader}>
              <div className="h-10 w-10 rounded-xl bg-[#F59E0B]/10 flex items-center justify-center text-[#F59E0B]">
                <BookOpen className="h-5 w-5" />
              </div>
              <h2 className="text-xl font-bold text-[#111827] dark:text-white">Entrance Exam Analysis</h2>
            </div>
            <div className="grid md:grid-cols-2 gap-5">
              {report.examAnalysis.map((exam, i) => (
                <div key={i} className={`${card} p-6 border-t-4 border-t-[#F59E0B]`}>
                  <h3 className="text-lg font-bold text-[#111827] dark:text-white mb-4">{exam.exam}</h3>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center border-b border-black/5 dark:border-white/5 pb-2">
                      <span className="text-sm text-[#6B7280]">Current Readiness</span>
                      <span className={`font-semibold ${exam.currentReadiness === "Pending" ? "text-amber-500" : "text-[#111827] dark:text-white"}`}>{exam.currentReadiness}</span>
                    </div>
                    <div className="flex justify-between items-center pb-2">
                      <span className="text-sm text-[#6B7280]">Recommended Prep</span>
                      <span className="font-bold text-[#F59E0B]">{exam.recommendedPrep}</span>
                    </div>
                  </div>
                </div>
              ))}
              {report.examAnalysis.length === 0 && (
                <div className="col-span-2 p-6 text-center text-[#6B7280]">
                  No specific entrance exams selected.
                </div>
              )}
            </div>
          </motion.section>

          {/* ══ SECTION 4: COLLEGE INTELLIGENCE ══ */}
          {showColleges && (
            <motion.section variants={itemVariants}>
              <div className={sectionHeader}>
                ...
                <div>
                  <h2 className="text-xl font-bold text-[#111827] dark:text-white">Bachelor College Intelligence</h2>
                  <p className="text-xs text-[#6B7280] mt-0.5">Projected based on your current readiness score</p>
                </div>
              </div>
                        ...
                        {colleges.map((c, i) => (
                          <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-black/[0.025] dark:bg-white/[0.025] rounded-xl border border-black/5 dark:border-white/5 gap-3">
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-[#6B7280] mt-0.5">Exam: {c.exam} · Tier {c.tier}</p>
                            </div>
                            <div className="flex items-center gap-6 shrink-0">
                              <div className="text-right hidden sm:block">
                                <p className="text-xs text-[#6B7280]">Rec. Score</p>
                                <p className="font-semibold text-sm text-[#111827] dark:text-white">{c.recommendedScore}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-xs text-[#6B7280]">Readiness</p>
                              ...
                              <div className="text-right min-w-[70px]">
                                ...
                                <p className={`font-bold text-sm ${c.gap === "On Track" ? "text-emerald-500" : c.gap === "Unknown" ? "text-amber-500" : "text-red-500"}`}>
                                  {c.gap}
                                </p>
                              </div>
                            </div>
          ...
          <motion.section variants={itemVariants}>
            <div className={sectionHeader}>
              ...
              <h2 className="text-xl font-bold text-[#111827] dark:text-white">Improvement Plan</h2>
            </div>
              ...
              {[
                ...
                <div key={key} className={`${card} p-6 border-t-4 ${border}`}>
                  ...
                  <ul className="space-y-3">
                    {report.improvementPlan[key].map((item, i) => (
          ...
          {/* ══ SECTION 7: MBA PATHWAY & FINAL ANSWERS ══ */}
          <motion.section variants={itemVariants} className="pb-20">
            <div className={sectionHeader}>
              <div className="h-10 w-10 rounded-xl bg-[#4F46E5]/10 flex items-center justify-center text-[#4F46E5]">
                <Star className="h-5 w-5" />
              </div>
              <h2 className="text-xl font-bold text-[#111827] dark:text-white">Your Long-Term MBA Pathway</h2>
            </div>
            <div className={`${card} p-8 mb-8 bg-gradient-to-br from-[#4F46E5]/5 to-transparent`}>
              <div className="flex items-center gap-4 flex-wrap text-sm font-bold text-[#4F46E5] mb-8">
                {report.finalAnswers.mbaPathway.split(" → ").map((step, i, arr) => (
                  <div key={i} className="flex items-center gap-4">
                    <span className="bg-white dark:bg-[#111827] px-4 py-2 rounded-full shadow-sm border border-black/5 dark:border-white/5">{step}</span>
                    {i < arr.length - 1 && <ArrowLeft className="h-4 w-4 rotate-180 text-[#6B7280]" />}
                  </div>
                ))}
              </div>
              
              <h3 className="text-lg font-bold text-[#111827] dark:text-white mb-4">Final Questions Answered</h3>
              <div className="space-y-4">
                {[
                  { q: "Which Bachelor's degree should I pursue?", a: report.finalAnswers.degree },
                  { q: "Which entrance exams should I take?", a: report.finalAnswers.exams },
                  { q: "Which colleges are Dream, Target, Safe?", a: report.finalAnswers.colleges },
                  { q: "What score do I need?", a: report.finalAnswers.scoreNeed },
                  { q: "What are my weaknesses?", a: report.finalAnswers.weakness },
                  { q: "How can I improve?", a: report.finalAnswers.improve }
                ].map((qa, i) => (
